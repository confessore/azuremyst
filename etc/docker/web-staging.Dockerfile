...
RUN dotnet build -c Staging --no-restore azuremyst.sln

FROM dotnet-build AS dotnet-test
RUN dotnet test -c Staging --no-build --no-restore azuremyst.sln

FROM dotnet-build AS publish
RUN dotnet publish -c Staging --no-build --no-restore -o /app  src/azuremyst/azuremyst.csproj
